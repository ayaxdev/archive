...
public class BreezilyStrafe implements StrafeCorrector {
    ...
    public void edit(float serverYaw, StrafeInputEvent event) {
        ...
        final Vec3d next = client.player.getPos().add(-Math.sin(Math.toRadians(angle)) * 0.5, 0, Math.cos(Math.toRadians(angle)) * 0.5);
        ...
        final double sollAngle = Math.toDegrees(Math.atan2(client.player.getX() - goTo.x, goTo.z - client.player.getZ()));
