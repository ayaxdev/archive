...
import com.atani.nextgen.event.impl.ItemRaytraceEvent;
...
public class Item
{
    ...
    protected MovingObjectPosition getMovingObjectPositionFromPlayer(World worldIn, EntityPlayer playerIn, boolean useLiquids)
    {
        ...
        if(playerIn == Minecraft.getMinecraft().thePlayer) {
            ItemRaytraceEvent itemRaytraceEvent = new ItemRaytraceEvent(f, f1);
            AtaniClient.getInstance().eventPubSub.publish(itemRaytraceEvent);
            f = itemRaytraceEvent.rotationPitch;
            f1 = itemRaytraceEvent.rotationYaw;
        }
