...
import com.atani.nextgen.event.impl.RenderPitchHeadEvent;
import com.atani.nextgen.event.impl.RenderYawHeadEvent;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T>
{
        ...
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, new Object[] {entity, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z)}))
        {
            ...
            try
            {
                ...
                if(entity == Minecraft.getMinecraft().thePlayer) {
                    RenderYawHeadEvent renderYawHeadEvent = new RenderYawHeadEvent(yaw);
                    AtaniClient.getInstance().eventPubSub.publish(renderYawHeadEvent);
                    yaw = renderYawHeadEvent.rotationYawHead;
                }
                ...
                if(entity == Minecraft.getMinecraft().thePlayer) {
                    RenderPitchHeadEvent renderPitchHeadEvent = new RenderPitchHeadEvent(f7, partialTicks);
                    AtaniClient.getInstance().eventPubSub.publish(renderPitchHeadEvent);
                    f7 = renderPitchHeadEvent.renderPitch;
                }
    ...
    public void renderName(T entity, double x, double y, double z) {
        ...
        AtaniClient.getInstance().eventPubSub.publish(nameTagRenderEvent);

        if(nameTagRenderEvent.cancelled) {
            return;
        }
            ...
            if (this.canRenderName(entity)) {
                ...
                if (d0 < (double)(f * f)) {
                    ...
                    if (entity.isSneaking()) {
                        ...
                        GlStateManager.enableTexture2D();
                        GlStateManager.depthMask(true);
                        fontrenderer.drawString(s, -fontrenderer.getStringWidth(s) / 2, 0, new Color(240,240,240).getRGB());
                        GlStateManager.enableLighting();
                        GlStateManager.disableBlend();
                        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
                        GlStateManager.popMatrix();
