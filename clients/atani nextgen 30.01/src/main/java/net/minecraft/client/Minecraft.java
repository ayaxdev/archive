...
public class Minecraft implements IThreadListener, IPlayerUsage
{
    ...
    {
        theMinecraft = this;
        AtaniClient.getInstance().preMinecraftLaunch();
    ...
    private void startGame() throws LWJGLException, IOException
    {
        ...
        AtaniClient.getInstance().postMinecraftLaunch();
    ...
    public void shutdownMinecraftApplet()
    {
        try
        {
            AtaniClient.getInstance().stopClient();
    ...
    private void runGameLoop() throws IOException
    {
        ...
        AtaniClient.getInstance().eventPubSub.publish(new GameLoopEvent(Event.EventType.PRE));

        TickShiftEvent tickShiftEvent = new TickShiftEvent(timer.elapsedTicks);
        AtaniClient.getInstance().eventPubSub.publish(tickShiftEvent);

        for (int j = 0; j < tickShiftEvent.ticks; ++j)
        {
            Event runTickEvent = new RunTickEvent(Event.EventType.PRE);
            AtaniClient.getInstance().eventPubSub.publish(runTickEvent);
            ...
            AtaniClient.getInstance().eventPubSub.publish(new RunTickEvent(Event.EventType.POST));
        }

        AtaniClient.getInstance().eventPubSub.publish(new GameLoopEvent(Event.EventType.POST));
    ...
    {
        AtaniClient.getInstance().eventPubSub.publish(new MouseClickRegisterEvent(Event.EventType.MID, 0));

        ClickCallEvent preEvent = new ClickCallEvent(Event.EventType.PRE, this.leftClickCounter);
        AtaniClient.getInstance().eventPubSub.publish(preEvent);
        ...
        ClickCallEvent postEvent = new ClickCallEvent(Event.EventType.POST, preEvent.leftClickCounter);
        AtaniClient.getInstance().eventPubSub.publish(postEvent);
    ...
    {
        AtaniClient.getInstance().eventPubSub.publish(new MouseClickRegisterEvent(Event.EventType.MID, 1));
    ...
    public void runTick() throws IOException
    {
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput)
        {
            ...
            while (Keyboard.next())
            {
                ...
                if (Keyboard.getEventKeyState())
                {
                    ...
                    {
                        AtaniClient.getInstance().eventPubSub.publish(new KeyPressEvent(k));
                        if (k == 1)
                        {
            ...
            {
                ToClickEvent toClickEvent = new ToClickEvent();
                AtaniClient.getInstance().eventPubSub.publish(toClickEvent);

                boolean clickedLeft = false, clickedRight = false;

                while (this.gameSettings.keyBindAttack.isPressed() || (toClickEvent.leftClick && !clickedLeft))
                {
                    MouseClickRegisterEvent mouseClickRegisterEvent = new MouseClickRegisterEvent(Event.EventType.PRE, 0);
                    AtaniClient.getInstance().eventPubSub.publish(mouseClickRegisterEvent);

                    if(mouseClickRegisterEvent.cancelled)
                        break;

                    this.clickMouse();
                    clickedLeft = true;

                    AtaniClient.getInstance().eventPubSub.publish(new MouseClickRegisterEvent(Event.EventType.POST, 0));
                }

                while (this.gameSettings.keyBindUseItem.isPressed() || (toClickEvent.rightClick && !clickedRight))
                {
                    MouseClickRegisterEvent mouseClickRegisterEvent = new MouseClickRegisterEvent(Event.EventType.PRE, 1);
                    AtaniClient.getInstance().eventPubSub.publish(mouseClickRegisterEvent);
                    ...
                    AtaniClient.getInstance().eventPubSub.publish(new MouseClickRegisterEvent(Event.EventType.POST, 1));
                }
        ...
        if (this.theWorld != null)
        {
            ...
            {
                AtaniClient.getInstance().eventPubSub.publish(new RunTickEvent(Event.EventType.MID));
