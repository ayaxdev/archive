...
import com.atani.nextgen.event.impl.EntityUpdateEvent;
import com.atani.nextgen.event.impl.KnockbackEvent;
import com.atani.nextgen.event.impl.UpdateDistanceEvent;
import com.atani.nextgen.event.impl.YawOffsetEvent;
...
public abstract class EntityLivingBase extends Entity
{
    ...
    public void onEntityUpdate()
    {
        ...
        if(this == Minecraft.getMinecraft().thePlayer) {
            EntityUpdateEvent entityUpdateEvent = new EntityUpdateEvent(this.prevRotationYawHead);
            AtaniClient.getInstance().eventPubSub.publish(entityUpdateEvent);

            this.prevRotationYawHead = entityUpdateEvent.prevRotationYawHead;
        }
            ...
            else
            {
                ...
                if (flag)
                {
                    ...
                    if (entity != null)
                    {
                        ...
                        if(this == Minecraft.getMinecraft().thePlayer)
                            AtaniClient.getInstance().eventPubSub.publish(knockbackEvent);
    ...
    public void onUpdate()
    {
        ...
        if (this.swingProgress > 0.0F)
        {
            ...
            if(this == Minecraft.getMinecraft().thePlayer) {
                YawOffsetEvent yawOffsetEvent = new YawOffsetEvent(this.rotationYaw);
                AtaniClient.getInstance().eventPubSub.publish(yawOffsetEvent);
                yaw = yawOffsetEvent.rotationYaw;
            }
    ...
    protected float updateDistance(float p_110146_1_, float p_110146_2_)
    {
        float yaw = this.rotationYaw;
        if(this == Minecraft.getMinecraft().thePlayer) {
            UpdateDistanceEvent updateDistanceEvent = new UpdateDistanceEvent(yaw);
            AtaniClient.getInstance().eventPubSub.publish(updateDistanceEvent);
            yaw = updateDistanceEvent.yaw;
        }
