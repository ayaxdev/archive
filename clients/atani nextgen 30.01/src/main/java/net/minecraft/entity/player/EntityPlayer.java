...
import com.atani.nextgen.event.impl.DeathEvent;
import com.atani.nextgen.event.impl.DropItemEvent;
import com.atani.nextgen.event.impl.KnockbackModifierEvent;
import com.atani.nextgen.event.impl.PigRotationEvent;
...
public abstract class EntityPlayer extends EntityLivingBase
{
    ...
    public void updateRidden()
    {
        ...
        else
        {
            ...
            if(this == Minecraft.getMinecraft().thePlayer) {
                PigRotationEvent pigRotationEvent = new PigRotationEvent(this.rotationYaw, this.rotationPitch);
                AtaniClient.getInstance().eventPubSub.publish(pigRotationEvent);
                f = pigRotationEvent.rotationYaw;
                f1 = pigRotationEvent.rotationPitch;
            }
            super.updateRidden();
    ...
    public void onDeath(DamageSource cause)
    {
        ...
        if(this == Minecraft.getMinecraft().thePlayer) {
            DeathEvent deathEvent = new DeathEvent(cause, this.rotationYaw);
            AtaniClient.getInstance().eventPubSub.publish(deathEvent);

            if(deathEvent.cancelled)
                return;

            cause = deathEvent.damageSource;
            yaw = deathEvent.rotationYaw;
        }
    ...
    public EntityItem dropItem(ItemStack droppedItem, boolean dropAround, boolean traceItem)
    {
        ...
        if(this == Minecraft.getMinecraft().thePlayer) {
            DropItemEvent dropItemEvent = new DropItemEvent(droppedItem, this.rotationYaw, this.rotationPitch);
            AtaniClient.getInstance().eventPubSub.publish(dropItemEvent);
            if(dropItemEvent.cancelled)
                return null;

            yaw = dropItemEvent.rotationYaw;
            pitch = dropItemEvent.rotationPitch;
        }
        ...
        else
        {
            ...
            else
            {
                ...
                entityitem.motionY = (double)(-MathHelper.sin(yaw / 180.0F * (float)Math.PI) * f2 + 0.1F);
            ...
            if (!targetEntity.hitByEntity(this))
            {
                ...
                if (f > 0.0F || f1 > 0.0F)
                {
                    ...
                    if (flag2)
                    {
                        ...
                        if(this == Minecraft.getMinecraft().thePlayer) {
                            KnockbackModifierEvent knockbackModifierEvent = new KnockbackModifierEvent(flag3, rotationYaw);
                            AtaniClient.getInstance().eventPubSub.publish(knockbackModifierEvent);

                            rotationYaw = knockbackModifierEvent.rotationYaw;
                            flag3 = knockbackModifierEvent.flag;
                        }

                        if (flag3)
                        {
                            targetEntity.addVelocity((double)(-MathHelper.sin(rotationYaw * (float)Math.PI / 180.0F) * (float)i * 0.5F), 0.1D, (double)(MathHelper.cos(rotationYaw * (float)Math.PI / 180.0F) * (float)i * 0.5F));
                            this.motionX *= 0.6D;
                            this.motionZ *= 0.6D;
