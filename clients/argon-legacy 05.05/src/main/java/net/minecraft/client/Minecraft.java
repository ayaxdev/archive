...
public class Minecraft implements IThreadListener, IPlayerUsage
{
    ...
    private void createDisplay() throws LWJGLException
    {
        ...
        final GameTitleEvent gameTitleEvent = new GameTitleEvent("Minecraft 1.8.9");
        Argon.getInstance().eventBus.post(gameTitleEvent);
        Display.setTitle(gameTitleEvent.title);
    ...
    public void displayGuiScreen(GuiScreen guiScreenIn)
    {
        ...
        final DisplayScreenEvent displayScreenEvent = new DisplayScreenEvent(guiScreenIn);
        Argon.getInstance().eventBus.post(displayScreenEvent);

        if (displayScreenEvent.cancelled)
            return;

        this.currentScreen = displayScreenEvent.guiScreen;

        if (displayScreenEvent.guiScreen != null)
        {
            this.setIngameNotInFocus();
            ScaledResolution scaledresolution = new ScaledResolution(this);
            int i = scaledresolution.getScaledWidth();
            int j = scaledresolution.getScaledHeight();
            displayScreenEvent.guiScreen.setWorldAndResolution(this, i, j);
    ...
    private void runGameLoop() throws IOException
    {
        ...
        {
            final RunTickEvent runTickEvent = new RunTickEvent(j);
            Argon.getInstance().eventBus.post(runTickEvent);
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput)
        {
            ...
            while (Keyboard.next())
            {
                ...
                if (Keyboard.getEventKeyState())
                {
                    ...
                    {
                        Argon.getInstance().eventBus.post(new HandleKeyEvent(k));
            ...
            Argon.getInstance().eventBus.post(new PreAttackEvent());
