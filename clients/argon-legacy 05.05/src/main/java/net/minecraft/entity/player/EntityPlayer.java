...
import ja.tabio.argon.event.impl.DeathEvent;
import ja.tabio.argon.event.impl.DropItemEvent;
import ja.tabio.argon.event.impl.KnockbackModifierEvent;
import ja.tabio.argon.event.impl.RiddenEntityUpdateEvent;
...
public abstract class EntityPlayer extends EntityLivingBase
{
    ...
    public void updateRidden()
    {
        ...
        else
        {
            ...
            final RiddenEntityUpdateEvent riddenEntityUpdateEvent = new RiddenEntityUpdateEvent(this, this.rotationYaw, this.rotationPitch);
            Argon.getInstance().eventBus.post(riddenEntityUpdateEvent);

            float f = riddenEntityUpdateEvent.rotationYaw;
            float f1 = riddenEntityUpdateEvent.rotationPitch;
    ...
    {
        final DeathEvent deathEvent = new DeathEvent(this, cause, this.rotationYaw);
        Argon.getInstance().eventBus.post(deathEvent);

        // God mode exploit?!
        if (deathEvent.cancelled)
            return;

        float yaw = deathEvent.rotationYaw;
    ...
    {
        final DropItemEvent event = new DropItemEvent(this, droppedItem, this.rotationYaw, this.rotationPitch);
        ...
        else
        {
            ...
            {
                float f2 = 0.3F;
                entityitem.motionX = (double)(-MathHelper.sin(event.rotationYaw / 180.0F * (float)Math.PI) * MathHelper.cos(event.rotationPitch / 180.0F * (float)Math.PI) * f2);
                entityitem.motionZ = (double)(MathHelper.cos(event.rotationYaw / 180.0F * (float)Math.PI) * MathHelper.cos(event.rotationPitch / 180.0F * (float)Math.PI) * f2);
                entityitem.motionY = (double)(-MathHelper.sin(event.rotationPitch / 180.0F * (float)Math.PI) * f2 + 0.1F);
        ...
        if (targetEntity.canAttackWithItem())
        {
            if (!targetEntity.hitByEntity(this))
            {
                ...
                if (f > 0.0F || f1 > 0.0F)
                {
                    ...
                    block: if (flag2)
                    {
                        final KnockbackModifierEvent knockbackModifierEvent = new KnockbackModifierEvent(this, i > 0, this.rotationYaw);
                        Argon.getInstance().eventBus.post(knockbackModifierEvent);

                        if (knockbackModifierEvent.cancelled)
                            break block;

                        if (knockbackModifierEvent.knockback)
                        {
                            targetEntity.addVelocity((double)(-MathHelper.sin(knockbackModifierEvent.rotationYaw * (float)Math.PI / 180.0F) * (float)i * 0.5F), knockbackModifierEvent.reduceY ? 0 : 0.1D, (double)(MathHelper.cos(rotationYaw * (float)Math.PI / 180.0F) * (float)i * 0.5F));
                            this.motionX *= 0.6D;
                            this.motionZ *= 0.6D;
