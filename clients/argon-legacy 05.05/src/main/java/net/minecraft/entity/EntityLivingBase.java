...
import ja.tabio.argon.event.impl.EntityHeadTurnDistanceEvent;
import ja.tabio.argon.event.impl.EntityHeadTurnEvent;
import ja.tabio.argon.event.impl.JumpEvent;
import ja.tabio.argon.event.impl.KnockbackEvent;
...
public abstract class EntityLivingBase extends Entity
{
            ...
            else
            {
                ...
                if (flag)
                {
                    ...
                    if (entity != null)
                    {
                        ...
                        final KnockbackEvent knockbackEvent = new KnockbackEvent(this, rotationYaw);
                        Argon.getInstance().eventBus.post(knockbackEvent);
    ...
    protected void jump()
    {
        final JumpEvent jumpEvent = new JumpEvent(this, this.rotationYaw, this.getJumpUpwardsMotion(), this.isPotionActive(Potion.jump), this.isSprinting());
        Argon.getInstance().eventBus.post(jumpEvent);

        if (jumpEvent.cancelled)
            return;

        this.motionY = jumpEvent.jumpHeight;

        if (jumpEvent.jumpBoost)
        {
            this.motionY += (double)((float)(this.getActivePotionEffect(Potion.jump).getAmplifier() + 1) * 0.1F);
        }

        if (jumpEvent.sprinting)
        {
    ...
    public void onUpdate()
    {
        ...
        {
            final EntityHeadTurnEvent event = new EntityHeadTurnEvent(this, rotationYaw);
            Argon.getInstance().eventBus.post(event);

            if (!event.cancelled)
                f1 = event.rotationYaw;
        }
    ...
    {
        final EntityHeadTurnDistanceEvent event = new EntityHeadTurnDistanceEvent(this, rotationYaw);
        Argon.getInstance().eventBus.post(event);

        float f = MathHelper.wrapAngleTo180_float(p_110146_1_ - this.renderYawOffset);
        this.renderYawOffset += f * 0.3F;
        float f1 = MathHelper.wrapAngleTo180_float(event.rotationYaw - this.renderYawOffset);
        ...
        this.renderYawOffset = event.rotationYaw - f1;
