...
import ja.tabio.argon.event.impl.BlockBBEvent;
...
public class Block
{
    ...
    public void addCollisionBoxesToList(World worldIn, BlockPos pos, IBlockState state, AxisAlignedBB mask, List<AxisAlignedBB> list, Entity collidingEntity)
    {
        ...
        final BlockBBEvent blockBBEvent = new BlockBBEvent(blockState.getBlock(), pos, axisalignedbb);
        Argon.getInstance().eventBus.post(blockBBEvent);

        if (blockBBEvent.cancelled)
            return;

        if (blockBBEvent.boundingBox != null && mask.intersectsWith(blockBBEvent.boundingBox))
        {
            list.add(blockBBEvent.boundingBox);
        }
    }
