...
import lord.daniel.alexander.event.impl.player.KnockbackModifierEvent;
...
public abstract class EntityPlayer extends EntityLivingBase
{
    ...
    {
        PlayerUpdateEvent playerUpdateEvent = new PlayerUpdateEvent(Event.Stage.PRE, onGround, posX, posY, posZ);
        Modification.getModification().getPubSub().publish(playerUpdateEvent);
        if(playerUpdateEvent.isCancelled())
            return;

        this.onGround = playerUpdateEvent.isOnGround();
        this.posX = playerUpdateEvent.getPosX();
        this.posY = playerUpdateEvent.getPosY();
        this.posZ = playerUpdateEvent.getPosZ();
        ...
        PlayerUpdateEvent postEvent = new PlayerUpdateEvent(Event.Stage.POST, onGround, posX, posY, posZ);
        Modification.getModification().getPubSub().publish(postEvent);
        if(postEvent.isCancelled())
            return;

        this.onGround = postEvent.isOnGround();
        this.posX = postEvent.getPosX();
        this.posY = postEvent.getPosY();
        this.posZ = postEvent.getPosZ();
    }
        ...
        if (targetEntity.canAttackWithItem())
        {
            if (!targetEntity.hitByEntity(this))
            {
                ...
                if (f > 0.0F || f1 > 0.0F)
                {
                    ...
                    if (flag2)
                    {
                        KnockbackModifierEvent knockbackModifierEvent = new KnockbackModifierEvent(this, i > 0);
                        Modification.getModification().getPubSub().publish(knockbackModifierEvent);
