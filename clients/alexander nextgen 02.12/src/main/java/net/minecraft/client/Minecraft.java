...
import lord.daniel.alexander.handler.game.GameEventHandler;
...
public class Minecraft implements IThreadListener, IPlayerUsage
{
    ...
    private void startGame() throws LWJGLException, IOException
    {
        ...
        Modification.getModification().startClient();
    ...
    public void shutdownMinecraftApplet()
    {
        try
        {
            Modification.getModification().endClient();
    ...
    {
        ClickEvent clickEvent = new ClickEvent(Event.Stage.PRE);
        Modification.getModification().getPubSub().publish(clickEvent);
        if(clickEvent.isCancelled())
            return false;
        if(clickEvent.isCancelVanillaClick())
            return true;
        ...
        Modification.getModification().getPubSub().publish(new ClickEvent(Event.Stage.POST));
        return true;
    }
    ...
    {
        RunTickEvent preEvent = new RunTickEvent(Event.Stage.PRE);
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput)
        {
            ...
            while (Keyboard.next())
            {
                ...
                if (Keyboard.getEventKeyState())
                {
                    ...
                    {
                        GameEventHandler.keyPress = true;
                        Modification.getModification().getPubSub().publish(new KeyInputEvent(k));
                        if (k == 1)
                        {
                        ...
                        GameEventHandler.keyPress = false;
                    }
            ...
            {
                Modification.getModification().getPubSub().publish(new RegisterClickingEvent(Event.Stage.PRE));

                while (this.gameSettings.keyBindAttack.isPressed())
                {
                    ClickHandler.incrementClick();
                }

                Modification.getModification().getPubSub().publish(new RegisterClickingEvent(Event.Stage.MID));
        ...
        if (this.theWorld != null)
        {
            ...
            {
                RunTickEvent midEvent = new RunTickEvent(Event.Stage.MID);
        ...
        RunTickEvent postEvent = new RunTickEvent(Event.Stage.POST);
        Modification.getModification().getPubSub().publish(postEvent);
    }
