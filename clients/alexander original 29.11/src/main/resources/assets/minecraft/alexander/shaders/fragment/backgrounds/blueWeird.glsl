...
const vec3 RGB = vec3(0.15, 0.65, 1.0);
...
void main() {
    vec3 sCoord = vec3(surfacePosition, 0.0);
    float sColour = 0.0;
    ...
    for (int i = 1; i <= 5; i++) {
        ...
        sNoiseAmount += (2.0 / power) * max(sNoise((sCoord * 0.1) + vec3(time * 0.0, time * 0.0, time * -0.01), power * sDensity), -1.0);
    }
    sNoiseAmount *= 0.5;
    ...
    sColour += sLevel;
    gl_FragColor = vec4(sFragColour, max(0.1, 1.0 - pow(sColour, 0.3)) * (1.0 - sLevel) * 1.0);
}