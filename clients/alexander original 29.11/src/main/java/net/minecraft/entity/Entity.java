...
import lord.daniel.alexander.module.impl.render.TrueSightModule;
import lord.daniel.alexander.storage.impl.ModuleStorage;
import lord.daniel.alexander.util.math.boundingbox.BBUtil;
...
public abstract class Entity implements ICommandSender
{
    ...
    {
        if(trueSightModule == null)
            trueSightModule = ModuleStorage.getModuleStorage().getByClass(TrueSightModule.class);

        return (player.isSpectator() && this.isInvisible()) || trueSightModule.isEnabled();
    }
    ...
    public double getDistanceToEntityBox(Entity entity) {
        ...
        Vec3 pos = BBUtil.getNearestPointBB(eyes, entity.getEntityBoundingBox());
    ...
    public double getLookDistanceToEntityBox(Entity entity, float[] rotation, double range) {
        Vec3 eyes = this.getPositionEyes(1F);
        Vec3 end = RotationUtil.toDirection(rotation != null ? rotation : new float[]{PlayerHandler.yaw, PlayerHandler.pitch}).multiply(range).add(eyes);
        MovingObjectPosition result = entity.getEntityBoundingBox().calculateIntercept(eyes, end);
        return (result != null && result.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) ? result.hitVec.distanceTo(eyes) : Double.MAX_VALUE;
    }

    public double getLookDistanceToEntityBox(Entity entity) {
        return getLookDistanceToEntityBox(entity, null, 10.0);
    }
