...
import lord.daniel.alexander.module.impl.exploit.AbortBreakingModule;
...
public class Minecraft implements IThreadListener, IPlayerUsage
{
    ...
    private void startGame() throws LWJGLException, IOException
    {
        ...
        Modification.INSTANCE.startClient();
    ...
    public void shutdownMinecraftApplet()
    {
        try
        {
            Modification.INSTANCE.endClient();
    ...
    private void runGameLoop() throws IOException
    {
        ...
        {
            PreTickEvent preTickEvent = new PreTickEvent();
            Modification.INSTANCE.getBus().post(preTickEvent);
        ...
        GameRendererEvent gameRendererEvent = new GameRendererEvent();
        Modification.INSTANCE.getBus().post(gameRendererEvent);

        if (!this.skipRenderWorld && !gameRendererEvent.isFreeze())
        {
    ...
    private AbortBreakingModule abortBreakingModule;

    public void sendClickBlockToController(boolean leftClick)
    {
        if(abortBreakingModule == null)
            abortBreakingModule = ModuleStorage.getModuleStorage().getByClass(AbortBreakingModule.class);
        ...
        if (this.leftClickCounter <= 0 && !this.thePlayer.isUsingItem())
        {
            ...
            else if(!abortBreakingModule.isEnabled()) {
    ...
    {
        Modification.INSTANCE.getBus().post(new RunTickEvent());

        if(!Modification.INSTANCE.getContinuousThread().isAlive())
            Modification.INSTANCE.genericExit();
        ...
        Modification.INSTANCE.getBus().post(new GuiHandleEvent());
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput)
        {
            ...
            while (Keyboard.next())
            {
                ...
                if (Keyboard.getEventKeyState())
                {
                    ...
                    {
                        Modification.INSTANCE.getBus().post(new KeyPressEvent(k));
                        if (k == 1)
                        {
            ...
            Modification.INSTANCE.getBus().post(new ClickingEvent());
            ...
            RightClickDelayEvent rightClickDelayEvent = new RightClickDelayEvent(this.rightClickDelayTimer);
            Modification.INSTANCE.getBus().post(rightClickDelayEvent);

            if (!rightClickDelayEvent.isCancelled() && this.gameSettings.keyBindUseItem.isKeyDown() && rightClickDelayEvent.getDelay() == 0 && !this.thePlayer.isUsingItem())
            {
                this.rightClickMouse();
            }
        ...
        if (this.theWorld != null)
        {
            ...
            {
                OnTickEvent tickEvent = new OnTickEvent();
                Modification.INSTANCE.getBus().post(tickEvent);
    ...
    {
        Modification.INSTANCE.getBus().post(new WorldLoadEvent(worldClientIn));
