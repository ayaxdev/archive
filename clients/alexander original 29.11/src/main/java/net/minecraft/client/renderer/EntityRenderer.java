...
import lord.daniel.alexander.module.impl.render.AmbienceModule;
import lord.daniel.alexander.module.impl.render.SmoothZoomModule;
...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void getMouseOver(float partialTicks)
    {
        ...
        if (entity != null && this.mc.theWorld != null)
        {
            ...
            Modification.INSTANCE.getBus().post(mouseOverEvent);
     ...
     */
    SmoothZoomModule smoothZoom = null;
    double zoomState = 1;

    private float getFOVModifier(float partialTicks, boolean p_78481_2_) {
        if(smoothZoom == null) {
            smoothZoom = ModuleStorage.getModuleStorage().getByClass(SmoothZoomModule.class);
        }
        double multiplier = smoothZoom.multiplier.getValue() / 10;
        if (this.debugView) {
            return 90.0F;
        } else {
            Entity entity = this.mc.getRenderViewEntity();
    ...
    public void func_181560_a(float p_181560_1_, long p_181560_2_)
    {
        ...
        if (this.mc.inGameHasFocus && flag)
        {
            if (this.pointedEntity instanceof EntityLivingBase) {
                ...
                final RendererEvent rendererEvent = new RendererEvent(livingBase.rotationYaw, livingBase.rotationPitch);
                Modification.INSTANCE.getBus().post(rendererEvent);
            }
            ...
            Modification.INSTANCE.getBus().post(entityRendererEvent);
            ...
            Modification.INSTANCE.getBus().post(rotationEvent);
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano)
    {
        ...
        Modification.INSTANCE.getBus().post(new Render3DEvent(partialTicks));
    ...
    private void addRainParticles()
    {
        if(ambienceModule == null)
            ambienceModule = ModuleStorage.getModuleStorage().getByClass(AmbienceModule.class);

        if(ambienceModule.isEnabled() && ambienceModule.getMinecraftWeather() == WeatherUtil.MinecraftWeather.SNOWY)
            return;
    ...
    private void setupFog(int p_78468_1_, float partialTicks)
    {
        ...
        else
        {
            ...
            RenderFogEvent fogEvent = new RenderFogEvent(Config.getFogStart());
            Modification.INSTANCE.getBus().post(fogEvent);

            if (p_78468_1_ == -1 && !fogEvent.isForceFog())
            {
                GlStateManager.setFogStart(0.0F);
            ...
            {
                GlStateManager.setFogStart(f3 * fogEvent.getFogMultiplier());
                GlStateManager.setFogEnd(f3);
            }
