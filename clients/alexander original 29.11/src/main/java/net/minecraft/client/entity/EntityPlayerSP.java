...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void onUpdate()
    {
        ...
        {
            UpdateMotionEvent preEvent = new UpdateMotionEvent(UpdateMotionEvent.Stage.PRE, posX, posY, posZ, onGround);
            Modification.INSTANCE.getBus().post(preEvent);
            super.onUpdate();

            if (this.isRiding())
            {
                UpdateMotionEvent midEvent = new UpdateMotionEvent(UpdateMotionEvent.Stage.MID, posX, posY, posZ, onGround);
                Modification.INSTANCE.getBus().post(midEvent);
                this.sendQueue.addToSendQueue(new C03PacketPlayer.C05PacketPlayerLook(PlayerHandler.yaw, PlayerHandler.pitch, midEvent.isOnGround()));
                this.sendQueue.addToSendQueue(new C0CPacketInput(this.moveStrafing, this.moveForward, this.movementInput.jump, this.movementInput.sneak));
                UpdateMotionEvent postEvent = new UpdateMotionEvent(UpdateMotionEvent.Stage.POST, posX, posY, posZ, onGround);
                Modification.INSTANCE.getBus().post(postEvent);
            }
            else
            {
                UpdateMotionEvent midEvent = new UpdateMotionEvent(UpdateMotionEvent.Stage.MID, posX, this.getEntityBoundingBox().minY, posZ, onGround);
                Modification.INSTANCE.getBus().post(midEvent);
                this.onUpdateWalkingPlayer(midEvent.getPosX(), midEvent.getPosY(), midEvent.getPosZ(), midEvent.isOnGround());
                UpdateMotionEvent postEvent = new UpdateMotionEvent(UpdateMotionEvent.Stage.POST, posX, posY, posZ, onGround);
                Modification.INSTANCE.getBus().post(postEvent);
            }
        }
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        ...
        MoveFlyingEvent moveFlyingEvent = new MoveFlyingEvent(strafe, forward, friction);
        Modification.INSTANCE.getBus().post(moveFlyingEvent);

        float yaw = correctMovement.isEnabled() ? PlayerHandler.yaw : this.rotationYaw;
    ...
    @Override
    public void jump() {
        JumpEvent jumpEvent = new JumpEvent(this.getJumpUpwardsMotion(), true);
        Modification.INSTANCE.getBus().post(jumpEvent);
        ...
        if (this.isPotionActive(Potion.jump) && jumpEvent.isAllowJumpBoost()) {
        ...
        if (this.isSprinting()) {
            if(correctMovement == null)
                correctMovement = ModuleStorage.getModuleStorage().getByClass(CorrectMovement.class);
            float f = correctMovement.isEnabled() ? PlayerHandler.yaw : this.rotationYaw;
     ...
     */
    public void onUpdateWalkingPlayer(double posX, double posY, double posZ, boolean onGround)
    {
        ...
        if (this.isCurrentViewEntity())
        {
            ...
            double d3 = (double)(PlayerHandler.yaw - this.lastReportedYaw);
            double d4 = (double)(PlayerHandler.pitch - this.lastReportedPitch);
            ...
            if (this.ridingEntity == null)
            {
                if (flag2 && flag3)
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(posX, posY, posZ, PlayerHandler.yaw, PlayerHandler.pitch, onGround));
                }
                else if (flag2)
                {
                ...
                {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C05PacketPlayerLook(PlayerHandler.yaw, PlayerHandler.pitch, onGround));
                }
                else
                {
            ...
            {
                this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.motionX, -999.0D, this.motionZ, PlayerHandler.yaw, PlayerHandler.pitch, onGround));
                flag2 = false;
            }
            ...
            if (flag3)
            {
                this.lastReportedYaw = PlayerHandler.yaw;
                this.lastReportedPitch = PlayerHandler.pitch;
            }
            --rotIncrement;
        }
    }
    ...
    public void sendChatMessage(String message)
    {
        ...
        Modification.INSTANCE.getBus().post(sendChatMessageEvent);
    ...
    {
        Modification.INSTANCE.getBus().post(new LivingUpdateEvent());
        ...
        Modification.INSTANCE.getBus().post(new SilentMoveEvent());

        if(correctMovement == null)
            correctMovement = ModuleStorage.getModuleStorage().getByClass(CorrectMovement.class);

        if (correctMovement.isEnabled() && correctMovement.silentMode.is("Direct")) {
            final float[] floats = this.applySilentMoveFix(this.movementInput.moveStrafe, this.movementInput.moveForward, this.rotationYaw, true);
        ...
        this.pushOutOfBlocks(this.posX + (double)this.width * 0.35D, this.getEntityBoundingBox().minY + 0.5D, this.posZ - (double)this.width * 0.35D);
        this.pushOutOfBlocks(this.posX + (double)this.width * 0.35D, this.getEntityBoundingBox().minY + 0.5D, this.posZ + (double)this.width * 0.35D);
        boolean flag3 = (float)this.getFoodStats().getFoodLevel() > 6.0 || this.capabilities.allowFlying;
        ...
        final DirectionSprintCheckEvent sprintCheckEvent = new DirectionSprintCheckEvent(this.movementInput.moveForward < f);
        Modification.INSTANCE.getBus().post(sprintCheckEvent);
    ...
    public float[] applySilentMoveFix(final float strafe, final float forward, final float yaw, final boolean advanced) {
        ...
        final double[] realMotion = MoveUtil.getMoveUtil().getMotion(0.22, strafe, forward, mc.thePlayer.rotationYaw);
        final double[] array;
        ...
        for (final float[] flo : possibleForwardStrafe) {
            ...
            final double[] motion = motion2 = MoveUtil.getMoveUtil().getMotion(0.22, flo[1], flo[0], PlayerHandler.yaw);
            final int n3 = 0;
    ...
    public int getPing() {
        return mc.getNetHandler() == null ? mc.getNetHandler().getPlayerInfo(getUniqueID()).getResponseTime() : 0;
    }
