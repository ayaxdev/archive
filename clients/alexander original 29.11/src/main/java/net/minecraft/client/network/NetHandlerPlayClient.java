...
import lord.daniel.alexander.event.impl.game.HeadLookEvent;
import lord.daniel.alexander.event.impl.game.PacketEvent;
import lord.daniel.alexander.event.impl.game.VelocityEvent;
...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    public void handleEntityVelocity(S12PacketEntityVelocity packetIn)
    {
        ...
        if (entity != null) {
            if(entity == Minecraft.getMinecraft().thePlayer) {
                Minecraft.getMinecraft().thePlayer.ticksSinceVelocity = 0;
            }

            double motionX = (double)packetIn.getMotionX() / 8000.0D,
                    motionY = (double)packetIn.getMotionY() / 8000.0D,
                    motionZ = (double)packetIn.getMotionZ() / 8000.0D;

            VelocityEvent velocityEvent = new VelocityEvent(entity, motionX, motionY, motionZ);
            Modification.INSTANCE.getBus().post(velocityEvent);

            if(velocityEvent.isCancelled())
                return;

            if(!velocityEvent.isIgnoreX())
                entity.motionX = velocityEvent.getMotionX();

            if(!velocityEvent.isIgnoreY())
                entity.motionY = velocityEvent.getMotionY();

            if(!velocityEvent.isIgnoreZ())
                entity.motionZ = velocityEvent.getMotionZ();
        }
    }
    ...
    public void handlePlayerPosLook(S08PacketPlayerPosLook packetIn)
    {
        ...
        final HeadLookEvent headLookEvent = new HeadLookEvent(f, f1, d0, d1, d2);
        Modification.INSTANCE.getBus().post(headLookEvent);
    ...
    {
        final PacketEvent packetEvent = new PacketEvent(p_147297_1_, this, PacketEvent.Stage.SENDING, null);
        Modification.INSTANCE.getBus().post(packetEvent);
