...
import com.skidding.atlas.util.render.font.FontCharacter;
...
public class ClientFontRenderer extends FontRenderer implements IMinecraft, Feature {
    ...
    private static final int INT_MAX_AMOUNT = 256;
    ...
    public int drawStringWithShadow(String text, float x, float y, int color) {
        this.drawString(text, x + .5f, y + .5f, color, true);
    ...
    public int drawString(String text, float x, float y, final int color, final boolean shadow) {
        ...
        final int shadowColor = this.shadowColor == 0 ? (color & 0xFCFCFC) >> 2 | color & 0xFF000000 : this.shadowColor;
            ...
            try {
                ...
                if (previousCharacter != COLOR_INVOKER) {
                    if (character == COLOR_INVOKER) {
                        int index = 21;
                        try {
                            index = COLOR_CODE_CHARACTERS.indexOf(text.toLowerCase().charAt(i + 1));
                        } catch (Exception _) { }
                        if (index < 16) {
                ...
                AtlasClient.getInstance().logger.error(STR."Character \{character} (\{(int) character}) was out of bounds for \{characterSet.length}", exception);
            }
