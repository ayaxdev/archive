...
public class ItemRenderer {
    ...
    public void renderItemInFirstPerson(float partialTicks) {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand()) {
            ...
            RenderItemUseEvent renderItemUseEvent = new RenderItemUseEvent();
            AtlasClient.getInstance().eventPubSub.publish(renderItemUseEvent);
            ...
            if (this.itemToRender != null) {
                if (this.itemToRender.getItem() instanceof ItemMap) {
                    ...
                    switch (enumaction) {
                        ...
                        case BLOCK:
                            RenderSwordUseEvent blockRenderEvent = new RenderSwordUseEvent(this, equippedProgress, 0.0F, swingProgress);
                            AtlasClient.getInstance().eventPubSub.publish(blockRenderEvent);
                            if (!blockRenderEvent.cancelled) {
                                this.transformFirstPersonItem(blockRenderEvent.equippedProgress, blockRenderEvent.renderSwingProgress);
                    ...
                    SmoothSwingAnimationEvent smoothSwingAnimationEvent = new SmoothSwingAnimationEvent(swingProgress);
                    AtlasClient.getInstance().eventPubSub.publish(smoothSwingAnimationEvent);
                    this.doItemUsedTransformations(smoothSwingAnimationEvent.renderSwingProgress);
