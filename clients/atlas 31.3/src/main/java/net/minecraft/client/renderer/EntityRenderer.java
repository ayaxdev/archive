...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime) {
        ...
        if (this.mc.inGameHasFocus && flag) {
            ...
            RotationEvent rotationEvent = new RotationEvent(mc.thePlayer.rotationYaw, mc.thePlayer.rotationPitch);
            AtlasClient.getInstance().eventPubSub.publish(rotationEvent);
        }
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
        AtlasClient.getInstance().eventPubSub.publish(new RenderWorldPassEvent(Event.EventType.PRE, partialTicks));
        ...
        AtlasClient.getInstance().eventPubSub.publish(new RenderWorldPassEvent(Event.EventType.MID, partialTicks));
        ...
        AtlasClient.getInstance().eventPubSub.publish(new Render3DEvent(partialTicks));
        ...
        AtlasClient.getInstance().eventPubSub.publish(new RenderWorldPassEvent(Event.EventType.POST, partialTicks));
    }
    ...
    private void addRainParticles() {
        ...
        final OverrideSnowEvent overrideSnowEvent = new OverrideSnowEvent();
        AtlasClient.getInstance().eventPubSub.publish(overrideSnowEvent);

        if (f != 0.0F && Config.isRainSplash() && !overrideSnowEvent.shouldOverride) {
                ...
                for (int l1 = i - i1; l1 <= i + i1; ++l1) {
                    ...
                    if (biomegenbase.canRain() || biomegenbase.getEnableSnow()) {
                        ...
                        if (k2 != l2) {
                            ...
                            if (world.getWorldChunkManager().getTemperatureAtHeight(f1, j2) >= 0.15F && !overrideSnowEvent.shouldOverride) {
                                if (j1 != 0) {
                                    if (j1 >= 0) {
                                        tessellator.draw();
                                    }
