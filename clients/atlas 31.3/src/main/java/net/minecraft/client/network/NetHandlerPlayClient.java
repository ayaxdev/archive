...
import com.skidding.atlas.event.impl.network.packet.HandleS08Event;
import com.skidding.atlas.event.impl.network.packet.HandleS18Event;
import com.skidding.atlas.event.impl.network.packet.S08XRotationEvent;
import com.skidding.atlas.event.impl.network.packet.S08YRotationEvent;
...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
    ...
    public void handleEntityTeleport(S18PacketEntityTeleport packetIn) {
        ...
        if (entity != null) {
            int posX = packetIn.getX(),
                    posY = packetIn.getY(),
                    posZ = packetIn.getZ();

            float rotationYaw = packetIn.getYaw(),
                    rotationPitch = packetIn.getPitch();

            if (entity == Minecraft.getMinecraft().thePlayer) {
                HandleS18Event handleS18Event = new HandleS18Event(Event.EventType.PRE, posX, posY, posZ, rotationYaw, rotationPitch, -1, -1);
                AtlasClient.getInstance().eventPubSub.publish(handleS18Event);

                posX = handleS18Event.posX;
                posY = handleS18Event.posY;
                posZ = handleS18Event.posZ;

                rotationYaw = handleS18Event.rotationYaw;
                rotationPitch = handleS18Event.rotationPitch;
            }
            ...
            float f = (float) (rotationYaw * 360) / 256.0F;
            float f1 = (float) (rotationPitch * 360) / 256.0F;
            ...
            if (entity == Minecraft.getMinecraft().thePlayer)
                AtlasClient.getInstance().eventPubSub.publish(new HandleS18Event(Event.EventType.POST, posX, posY, posZ, rotationYaw, rotationPitch, f, f1));
    ...
    public void handlePlayerPosLook(S08PacketPlayerPosLook packetIn) {
        ...
        if (packetIn.func_179834_f().contains(S08PacketPlayerPosLook.EnumFlags.X_ROT)) {
            S08XRotationEvent s08XRotationEvent = new S08XRotationEvent(entityplayer.rotationPitch);
            AtlasClient.getInstance().eventPubSub.publish(s08XRotationEvent);
            f1 += s08XRotationEvent.rotationPitch;
        }

        if (packetIn.func_179834_f().contains(S08PacketPlayerPosLook.EnumFlags.Y_ROT)) {
            S08YRotationEvent s08YRotationEvent = new S08YRotationEvent(entityplayer.rotationYaw);
            AtlasClient.getInstance().eventPubSub.publish(s08YRotationEvent);
            f += s08YRotationEvent.rotationYaw;
        }

        HandleS08Event preEvent = new HandleS08Event(Event.EventType.PRE, f, f1);
        AtlasClient.getInstance().eventPubSub.publish(preEvent);
        entityplayer.setPositionAndRotation(d0, d1, d2, preEvent.rotationYaw, preEvent.rotationPitch);

        HandleS08Event midEvent = new HandleS08Event(Event.EventType.MID, preEvent.rotationYaw, preEvent.rotationPitch);
        AtlasClient.getInstance().eventPubSub.publish(midEvent);
        this.netManager.sendPacket(new C03PacketPlayer.C06PacketPlayerPosLook(entityplayer.posX, entityplayer.getEntityBoundingBox().minY, entityplayer.posZ, midEvent.rotationYaw, midEvent.rotationPitch, false));

        HandleS08Event postEvent = new HandleS08Event(Event.EventType.POST, midEvent.rotationYaw, midEvent.rotationPitch);
    ...
    public void addToSendQueue(Packet p_147297_1_) {
        HandlePacketEvent handlePacketEvent = new HandlePacketEvent(Event.EventType.OUTGOING, p_147297_1_);
        AtlasClient.getInstance().eventPubSub.publish(handlePacketEvent);

        if (!handlePacketEvent.cancelled)
