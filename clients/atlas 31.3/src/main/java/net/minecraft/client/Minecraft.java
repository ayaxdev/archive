...
import com.skidding.atlas.event.impl.game.TickShiftEvent;
...
import com.skidding.atlas.util.animation.deprecated.AnimationUtil;
...
public class Minecraft implements IThreadListener, IPlayerUsage {
    ...
    private PlayerUsageSnooper usageSnooper = new PlayerUsageSnooper(this, MinecraftServer.getCurrentTimeMillis());
    ...
    public Minecraft(GameConfiguration gameConfig) {
        theMinecraft = this;
        AtlasClient.getInstance().preMinecraftLaunch();
    ...
    private void startGame() throws LWJGLException, IOException {
        ...
        AtlasClient.getInstance().postMinecraftLaunch();
    ...
    public void displayGuiScreen(GuiScreen guiScreenIn) {
        ScreenOpenEvent event = new ScreenOpenEvent(guiScreenIn);
        AtlasClient.getInstance().eventPubSub.publish(event);
    ...
    public void shutdownMinecraftApplet() {
        try {
            AtlasClient.getInstance().stopClient();
    ...
    private void runGameLoop() throws IOException {
        AtlasClient.getInstance().eventPubSub.publish(new GameLoopEvent(Event.EventType.PRE));

        long currentTime = getTime();
        AnimationUtil.delta = currentTime - this.lastMS;
        ...
        AtlasClient.getInstance().eventPubSub.publish(new GameLoopEvent(Event.EventType.MID));

        TickShiftEvent tickShiftEvent = new TickShiftEvent(timer.elapsedTicks);
        AtlasClient.getInstance().eventPubSub.publish(tickShiftEvent);

        for (int j = 0; j < tickShiftEvent.ticks; ++j) {
            Event runTickEvent = new RunTickEvent(Event.EventType.PRE);
            AtlasClient.getInstance().eventPubSub.publish(runTickEvent);

            if (runTickEvent.cancelled)
                continue;

            this.runTick();

            AtlasClient.getInstance().eventPubSub.publish(new RunTickEvent(Event.EventType.POST));
        }

        AtlasClient.getInstance().eventPubSub.publish(new GameLoopEvent(Event.EventType.POST));
    ...
    public void clickMouse() {
        AtlasClient.getInstance().eventPubSub.publish(new RegisterClickEvent(Event.EventType.MID, 0));

        RunClickEvent preEvent = new RunClickEvent(Event.EventType.PRE, this.leftClickCounter);
        AtlasClient.getInstance().eventPubSub.publish(preEvent);

        if (preEvent.leftClickCounter <= 0) {
            //this.thePlayer.swingItem();
        ...
        RunClickEvent postEvent = new RunClickEvent(Event.EventType.POST, preEvent.leftClickCounter);
        AtlasClient.getInstance().eventPubSub.publish(postEvent);
        this.leftClickCounter = postEvent.leftClickCounter;
    }

    @SuppressWarnings("incomplete-switch")
    public void rightClickMouse() {
        AtlasClient.getInstance().eventPubSub.publish(new RegisterClickEvent(Event.EventType.MID, 1));
    ...
    private void updateFramebufferSize() {
        ...
        AtlasClient.getInstance().eventPubSub.publish(new ResolutionChangeEvent());
    }
    ...
    public void runTick() throws IOException {
        ...
        AtlasClient.getInstance().eventPubSub.publish(new ScreenClickEvent());
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput) {
            ...
            while (Keyboard.next()) {
                ...
                if (Keyboard.getEventKeyState()) {
                    ...
                    if (this.currentScreen != null) {
                        ...
                        AtlasClient.getInstance().eventPubSub.publish(new KeyPressEvent(k));
                        if (k == 1) {
            ...
            if (this.thePlayer.isUsingItem()) {
                ...
                WaitForClickEvent waitForClickEvent = new WaitForClickEvent();
                AtlasClient.getInstance().eventPubSub.publish(waitForClickEvent);

                boolean clickedLeft = false, clickedRight = false;

                while (this.gameSettings.keyBindAttack.isPressed() || (waitForClickEvent.leftClick && !clickedLeft)) {
                    RegisterClickEvent registerClickEvent = new RegisterClickEvent(Event.EventType.PRE, 0);
                    AtlasClient.getInstance().eventPubSub.publish(registerClickEvent);

                    if (registerClickEvent.cancelled)
                        break;

                    this.clickMouse();
                    clickedLeft = true;

                    AtlasClient.getInstance().eventPubSub.publish(new RegisterClickEvent(Event.EventType.POST, 0));
                }

                while (this.gameSettings.keyBindUseItem.isPressed() || (waitForClickEvent.rightClick && !clickedRight)) {
                    RegisterClickEvent registerClickEvent = new RegisterClickEvent(Event.EventType.PRE, 1);
                    AtlasClient.getInstance().eventPubSub.publish(registerClickEvent);
                    ...
                    AtlasClient.getInstance().eventPubSub.publish(new RegisterClickEvent(Event.EventType.POST, 1));
                }
        ...
        if (this.theWorld != null) {
            if (this.thePlayer != null) {
                AtlasClient.getInstance().eventPubSub.publish(new RunTickEvent(Event.EventType.MID));
    ...
    public void loadWorld(WorldClient worldClientIn, String loadingMessage) {
        AtlasClient.getInstance().eventPubSub.publish(new LoadWorldEvent());
