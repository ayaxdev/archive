...
public abstract class EntityPlayer extends EntityLivingBase {
    ...
    public void updateRidden() {
        if (!this.worldObj.isRemote && this.isSneaking()) {
            ...
            if (this == Minecraft.getMinecraft().thePlayer) {
                ...
                AtlasClient.getInstance().eventPubSub.publish(pigRotationEvent);
    ...
    public void onDeath(DamageSource cause) {
        ...
        if (this == Minecraft.getMinecraft().thePlayer) {
            ...
            AtlasClient.getInstance().eventPubSub.publish(deathEvent);
    ...
    public EntityItem dropItem(ItemStack droppedItem, boolean dropAround, boolean traceItem) {
        ...
        if (this == Minecraft.getMinecraft().thePlayer) {
            ...
            AtlasClient.getInstance().eventPubSub.publish(dropItemEvent);
            ...
            if (!targetEntity.hitByEntity(this)) {
                ...
                if (f > 0.0F || f1 > 0.0F) {
                    ...
                    if (flag2) {
                        ...
                        boolean flag3 = i > 0;
                        boolean reducedY = false;

                        if (this == Minecraft.getMinecraft().thePlayer) {
                            KnockbackModifierEvent knockbackModifierEvent = new KnockbackModifierEvent(flag3, rotationYaw);
                            AtlasClient.getInstance().eventPubSub.publish(knockbackModifierEvent);

                            rotationYaw = knockbackModifierEvent.rotationYaw;
                            flag3 = knockbackModifierEvent.flag;
                            reducedY = knockbackModifierEvent.reduceY;
                        }

                        if (flag3) {
                            targetEntity.addVelocity((double) (-MathHelper.sin(rotationYaw * (float) Math.PI / 180.0F) * (float) i * 0.5F), reducedY ? 0 : 0.1D, (double) (MathHelper.cos(rotationYaw * (float) Math.PI / 180.0F) * (float) i * 0.5F));
                            KeepSprintEvent keepSprintEvent = new KeepSprintEvent();
                            AtlasClient.getInstance().eventPubSub.publish(keepSprintEvent);
                            if (!keepSprintEvent.cancelled) {
                                this.motionX *= 0.6D;
                                this.motionZ *= 0.6D;
