...
public abstract class EntityLivingBase extends Entity {
    ...
    public void onEntityUpdate() {
        ...
        if (this == Minecraft.getMinecraft().thePlayer) {
            EntityTickEvent entityTickEvent = new EntityTickEvent(this.prevRotationYawHead);
            AtlasClient.getInstance().eventPubSub.publish(entityTickEvent);

            this.prevRotationYawHead = entityTickEvent.prevRotationYawHead;
        }
            ...
            if (this.getHealth() <= 0.0F) {
                ...
                if (flag) {
                    ...
                    if (entity != null) {
                        ...
                        KnockbackEvent knockbackEvent = new KnockbackEvent(this.rotationYaw);

                        if (this == Minecraft.getMinecraft().thePlayer)
                            AtlasClient.getInstance().eventPubSub.publish(knockbackEvent);

                        if (!knockbackEvent.cancelled) {
                            this.attackedAtYaw = (float) (MathHelper.atan2(d0, d1) * 180.0D / Math.PI - (double) knockbackEvent.rotationYaw);
    ...
    private int getArmSwingAnimationEnd() {
        ArmSwingAnimationEvent speedModifierEvent = new ArmSwingAnimationEvent(1.0);
        AtlasClient.getInstance().eventPubSub.publish(speedModifierEvent);
        ...
        return this.isPotionActive(Potion.digSpeed)
                ...
                : 6) * (1.0 / speedModifierEvent.swingSpeed));
    }
    ...
    public void onUpdate() {
        ...
        if (this.swingProgress > 0.0F) {
            ...
            if (this == Minecraft.getMinecraft().thePlayer) {
                HeadTurnEvent headTurnEvent = new HeadTurnEvent(this.rotationYaw);
                AtlasClient.getInstance().eventPubSub.publish(headTurnEvent);
                yaw = headTurnEvent.rotationYaw;
            }
    ...
    protected float updateDistance(float p_110146_1_, float p_110146_2_) {
        float yaw = this.rotationYaw;
        if (this == Minecraft.getMinecraft().thePlayer) {
            RenderPitchHeadEvent.HeadTurnDistanceEvent headTurnDistanceEvent = new RenderPitchHeadEvent.HeadTurnDistanceEvent(yaw);
            AtlasClient.getInstance().eventPubSub.publish(headTurnDistanceEvent);
            yaw = headTurnDistanceEvent.yaw;
        }
    ...
    public void onLivingUpdate() {
        ...
        if (this.isJumping) {
            if (this.isInWater()) {
                ...
                this.jump();
                JumpTicksEvent jumpTicksEvent = new JumpTicksEvent(10);
                AtlasClient.getInstance().eventPubSub.publish(jumpTicksEvent);

                this.jumpTicks = jumpTicksEvent.jumpTicks;
            }
        } else {
            this.jumpTicks = 0;
        }
